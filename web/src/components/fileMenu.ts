...
export function menuFor(e: Entry, h: MenuHandlers): DropdownItem[] {
  ...
  if (e.kind === "folder") {
    return [
      { label: t("drive.rename"), icon: Pencil, onClick: () => h.renameFolder(e.folder.id, e.folder.name) },
      { label: t("drive.move"), icon: FolderInput, onClick: () => h.moveFolder(e.folder.id) },
      { label: t("common.delete"), icon: Trash2, danger: true, onClick: () => h.deleteFolder(e.folder.id, e.folder.name) },
    ];
  }
  const f = e.file;
  return [
    ...
    { label: t("common.delete"), icon: Trash2, danger: true, onClick: () => h.deleteFile(f) },
  ];
}
